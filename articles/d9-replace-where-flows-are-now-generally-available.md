...
検証を通して、ドキュメントを読むだけでは気づきにくい点がいくつかありました。

- **REPLACE WHERE フローは SQL エディタ単体では実行できない。** `CREATE FLOW ... REPLACE WHERE` は Lakeflow 宣言的パイプラインの中でのみ評価されます。通常の SQL 実行や Statement Execution API に投げると `UC_COMMAND_NOT_SUPPORTED` で失敗します。パイプライン専用の構文だと割り切って設計する必要があります。
- **1 つのストリーミングテーブルに REPLACE WHERE フローは 1 本まで。** 「関東用・関西用…と地域ごとにフローを分けて 1 テーブルに集約する」といった構成はできず、`UNSUPPORTED_FEATURE.MULTI_FLOW_REPLACE_WHERE_TARGET` になります。範囲は 1 本の述語で表現します。
- **実行モードはイベントログで裏取りできる。** 今回は `COMPLETE_REPLACE_WHERE` として計画されたことがログに残りました。意図した範囲置換が本当に行われたかを、あとから確認できます。
- **パイプラインの出力先スキーマは、ソースと別のスキーマにする。** 同じスキーマを指定すると、パイプラインの内部生成物がソースと衝突し `Found duplicate dataset` で失敗します。
